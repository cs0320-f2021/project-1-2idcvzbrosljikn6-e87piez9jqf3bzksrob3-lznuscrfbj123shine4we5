...
public class ApiClientTest {
  ...
  public void testUsersCall() {
    ...
    assertEquals(DataStore.getUsers().length, 15, 2);

    ac.usersApiCall(true);
    assertEquals(DataStore.getRunways().length, 15, 2);
  }
  ...
  public void testRentsCall() {
    ApiClient ac = new ApiClient();
    ac.rentsApiCall();
    assertEquals(DataStore.getRents().length, 30, 2);
  }
  ...
  public void testReviewsCall() {
    ApiClient ac = new ApiClient();
    ac.reviewsApiCall();
    assertEquals(DataStore.getReviews().length, 30, 2);
  }
